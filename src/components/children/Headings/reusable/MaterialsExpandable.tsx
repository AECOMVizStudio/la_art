  ...
  return (
    <div className="mb-4">
        ...
        <div className={expanded ? 'grid grid-cols-4 gap-0' : 'hidden'}>
          {props.map((material) => {
            return (
              <div
                key={material.key}
                className="flex justify-start items-center sm:items-stretch p-1 sm:p-2 text-blue-500 underline border-2"
              >
                <a
                  href={material.ref}
                  target="_blank"
      ...
      <div className="hidden min-w-full ">
        <p className="px-2 py-4 border-y-4 font-bold w-1/4">
          {props[0].document}
        </p>
        {props.map((material) => {
          return (
            <div
              className="border-t-4 border-b-4 border-l-2 flex items-center justify-center lg:px-2 xl:px-4"
              key={material.key}
            >
              <a
                href={material.ref}
